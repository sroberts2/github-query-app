...
import uk.gov.companieshouse.githubapi.model.GitHubAppData;
...
public class AppDataService {
    ...
    /**
     * Load the app data from the database.
     * @return a list of GitHubAppData
     */
    ...
    private GitHubAppData convertToGitHubAppData(final GitHubRepositoryDao repository) {
        return resolvePossibleSpringBootVersion(repository.getDependencies()).map(
            springBootVersion -> new GitHubAppData(
                    repository.getRepoName(),
                    repository.getRepoName(),
                    springBootVersion
            )
        ).orElseGet(() -> new GitHubAppData(
                repository.getRepoName(),
                repository.getRepoName(),
                SPRING_VERSION_NOT_RESOLVED)
        );
    }

    private Optional<String> resolvePossibleSpringBootVersion(
            final Map<String, String> dependencies
    ) {
        final Optional<String> springBootVersion = resolveDependencyVersion(
                dependencies, SPRING_BOOT_DEPENDENCY_PREFIX
        );
        ...
        final Optional<String> springFrameworkVersion = resolveDependencyVersion(
                dependencies, SPRING_FRAMEWORK_DEPENDENCY_PREFIX
        ...
        return resolveDependencyVersion(dependencies, "companies-house-parent")
            .map(version -> 
                    "Uses companies-house-parent pom (version: "
                    + version
                    + "), no Spring version detected"
            );
    }

    private final Optional<String> resolveDependencyVersion(
            final Map<String, String> dependencies,
            final String dependencyPrefix
    ) {
